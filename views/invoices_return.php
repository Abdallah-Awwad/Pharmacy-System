    ...
    <div class="main-page" id="all_invoices">
        <div class="return-invoices">
            <h1>
                <span><?= $lang["Return invoices"] ?></span>
            </h1>
    ...
    <script>
        $(document).ready(function() {
            requestAjax({'process' : 'readReturnInvoices'}, function (result) {
                if (result == "[]") {
                    ...
                    $.each(result, function (serial, value) { 
                        ...
                        for (i = 0; i < Object.values(value).length; i++) {
                            ...
                            tr = '<tr onclick=window.location.href="invoice_view?id=' + value["id"] + '"> </td>' + td + '</tr>';
                        }
                        $("tbody").append(tr);
                    });
                    $("table").addClass("sort");
