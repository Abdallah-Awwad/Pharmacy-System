    ...
    <script> 
        $(document).ready(function(){
            let bindValues = {
                'process': 'readInvoice',
                'invoiceID': (new URLSearchParams((new URL(window.location.href)).search)).get('id')
            }
            ...
            let bindValues2 = {
                'process': 'readInvoiceDetails',
                'invoiceID': (new URLSearchParams((new URL(window.location.href)).search)).get('id')
            }
            requestAjax(bindValues2, function(result){
                if (result == "[]"){
                    window.location.href = "dashboard";
                }
                else{
